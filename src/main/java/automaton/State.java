...
public class State {
    ...
    @Deprecated
    public List<State> transactUsingS3(EventSchema schema, byte[] eventRecord, long timeWindows){
    ...
    /**
     * when an event arrival, check whether current state can transact next state
     * @param schema        event schema
     * @param eventRecord   event
     * @return              next transaction state set
     */
    @Deprecated
    public List<State> transactUsingS2(EventSchema schema, byte[] eventRecord, long timeWindows){
        List<State> nextStates = new ArrayList<>();
        // skip till any match
        nextStates.add(this);
        int timeIdx = schema.getTimestampIdx();
                ...
                if(isStart){
                    ...
                    while(it.hasNext()){
                        ...
                        if(timeout){
                            ...
                            if(satisfyAllDC){
                                // if satisfy all independent constraints and dependent constraints
                                // then previous partial match must be deleted
                                // the only difference for skip-till-any-match and skip-till-next-match is here
                                it.remove();
                                // create a match and add it to next buffer
                                List<Long> timeList = new ArrayList<>(curMatch.timeList());
                                timeList.add(timestamp);
